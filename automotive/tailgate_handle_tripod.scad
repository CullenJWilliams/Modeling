...
{
		difference(){
			translate([0,0,-shaft_r-6]) cylinder(shaft_h,shaft_r+5,shaft_r+5);
			shaft();
		}
...
{
	translate([0,0,16+r]) rotate([0,90,0]) cylinder(shaft_r,r+gap,r+gap);
	translate([shaft_r,0,16+r]) rotate([0,90,0]) sphere(r+gap);
